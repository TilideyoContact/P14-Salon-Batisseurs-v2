...
export function Hero() {
  return (
    <div className="relative overflow-hidden rounded-b-3xl">
      ...
      <div className="relative z-10 py-12 md:py-20 px-4 mt-14">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-display mb-4 leading-tight text-white">
            Configurez votre<br />
            <span className="text-primary">formation sur mesure</span>
          </h1>

          <p className="text-base sm:text-lg md:text-xl text-white/70 max-w-2xl mx-auto mb-8 sm:mb-10 text-balance">
...
function Stat({ value, label, highlight = false }: { value: string; label: string; highlight?: boolean }) {
  return (
    <div className={`px-3 sm:px-5 py-2 sm:py-3 flex flex-col items-center min-w-[80px] sm:min-w-[110px] rounded-2xl border border-white/15 bg-white/10 backdrop-blur-sm ${
      highlight ? "border-primary/50 shadow-[0_0_15px_rgba(198,30,90,0.2)]" : ""
    }`}>
      <span className={`text-xl sm:text-2xl font-bold font-display ${highlight ? "text-primary" : "text-white"}`}>
        {value}
      </span>
